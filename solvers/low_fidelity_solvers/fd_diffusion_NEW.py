...
def laplacian(conductivity, h=1.0):
    ...
    # Define centered difference stencil
    dx_main = -2 * np.ones(N - 1)   # Main diagonal
    dx_upper = np.ones(N - 2)       # Upper diagonal
    dx_lower = np.ones(N - 2)       # Lower diagonal

    # Create sparse matrix for 1D Laplacian
    D = sp.diags([dx_main, dx_upper, dx_lower], [0, 1, -1], shape=(N - 1, N - 1), format="csr")
    ...
    index = (N_cells/2)*N_cells+N_cells/2
    ...
    # Flatten
    conductivity_flat = conductivity.reshape(batch_size, N_cells ** 2).flatten()

    C_batch = sp.diags([conductivity_flat], [0], shape=(batch_size * N_cells**2, batch_size * N_cells**2), format="csr")
...
def fd_bwd(res, grads):
    ...
    dl_dK = dl_dT @ lambd_reshape / 150.0
